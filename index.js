...
//event listeners for buttons
  ...
  //object to store calc functions for each operator
    ...
    function positiveNegative(numString) {
      //if "-" is present in numString, split, splice and take out "-", then join.
    ...
    for (let i = 0; i < allOperators.length; i++){
      allOperators[i].addEventListener('click', (e) => {
        ...
        //if activeOperand is false and "-" is present already, run displayNum func
    ...
    pN.addEventListener('click', () => {
      //if result is true and operand1 isn't, set operand1 to result to prevent multiple clicks of -
      if (result && !operand1) {
        operand1 = result;
      }

      //if operator is clicked, run positiveNegative func on operand2
      if (operator) {
      ...
      //else, run positiveNegative func on operand2
